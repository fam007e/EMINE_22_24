...
def Pitch(rho, v, c_p, chi_ave=20e3):
    T_clad = 820 #K
    T_in = 670 #K
    del_T = T_clad - T_in
    L_fuel = 100e-2 #m
...
def h(rho, v, c_p, k, P, chi_ave=20e3):
    T_clad = 820 #K
    T_in = 670 #K
    del_T = T_clad - T_in
    L_fuel = 100e-2 #m
...
def T_out(rho, v, c_p, chi_ave=20e3):
    T_clad = 820 #K
    T_in = 670 #K
    del_T = T_clad - T_in
    L_fuel = 100e-2 #m
...
print(f"Heat transfer coefficient, h\nFor Na:\nh_begin:{h_beginNa:.4f}\nh_end:{h_endNa:.4f}\nFor Pb:\nh_begin:{h_beginPb:.4f}\nh_end:{h_endPb:.4f}")

def P_ch(rho, v, mu, c_p, chi_ave=1.5e3): # massive problem with the rho here? We have rho_hot and rho_cold and rho so what are we taking into account for H_HX
    T_clad = 820 #K
    T_in = 670 #K
    del_T = T_clad - T_in
    L_fuel = 100e-2 #m
